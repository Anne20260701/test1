...
def generate_eeg_data(duration=10, fs=256, amplitude=20, latency=300, noise_level=0.5, is_abnormal=False):
    ...
    background = alpha + theta + beta + noise
    ...
    for stim_time in stimulus_times:
        ...
        if idx < len(t) - 250:
            ...
            # 生成ERP波形（直接添加到EEG中）
            for i in range(-50, 200):  # -50ms 到 200ms
                pos = idx + i
                if 0 <= pos < len(t):
                    time_ms = i / fs * 1000
                    ...
                    # P300：在latency位置出现
                    p300_peak = latency
                    p300_val = amplitude * np.exp(-((time_ms - p300_peak) ** 2) / (2 * p300_width ** 2))

                    # N100：在100ms位置
                    n100_val = -amplitude * 0.5 * np.exp(-((time_ms - 100) ** 2) / (2 * n100_width ** 2))

                    # P200：在180ms位置
                    p200_val = amplitude * 0.3 * np.exp(-((time_ms - 180) ** 2) / (2 * p200_width ** 2))

                    # 异常模式：振幅降低、潜伏期延长
                    if is_abnormal:
                        p300_val = p300_val * 0.65
                        # 潜伏期延长通过改变峰值位置来实现
                        p300_shifted = amplitude * 0.65 * np.exp(
                            -((time_ms - (latency + 25)) ** 2) / (2 * p300_width ** 2))
                        p300_val = p300_shifted

                    eeg_data[pos] += p300_val + n100_val + p200_val
...
def extract_erp(eeg_data, stimulus_times, fs, pre_stim=0.2, post_stim=0.8):
    """第三步，提取ERP成分：从连续EEG中切割刺激前后的片段并平均"""
...
def calculate_risk_score(p300_amplitude, p300_latency, complaint_severity):
    """第五步，输出风险评分：计算AI-Fraud Score（行为-神经不匹配指数）"""
    ...
    # 计算神经异常程度
    # 振幅异常（振幅越低越异常）
    amplitude_abnormality = max(0, min(50, (normal_amplitude - p300_amplitude) / normal_amplitude * 50) )
    # 潜伏期异常（潜伏期越长越异常）
    latency_abnormality = max(0, min(50, (p300_latency - normal_latency) / normal_latency * 50) )

    neural_abnormality = min(100, amplitude_abnormality + latency_abnormality)

    # 行为-神经不匹配 = 主诉严重程度 - 神经异常程度
    mismatch = max(0, complaint_severity - neural_abnormality * 0.7)

    # AI-Fraud Score (0-1000)
    fraud_score = min(1000, mismatch * 7 + neural_abnormality * 3)
...
if run_analysis:
    with st.spinner("🧠 正在分析EEG信号..."):
        ...
        # 根据场景设置参数
        if scenario == "健康人（基线）":
            amp_factor, lat_shift, abnormal = 1.0, 0, False
        elif scenario == "疑似脑震荡（异常）":
            amp_factor, lat_shift, abnormal = 0.7, 25, True
        else:  # 装病
            amp_factor, lat_shift, abnormal = 0.9, 8, False

        base_amplitude = 20 * amp_factor
        base_latency = 300 + lat_shift

        actual_amplitude = base_amplitude + np.random.normal(0, 1.5)
        actual_amplitude = max(8, min(22, actual_amplitude))

        actual_latency = base_latency + np.random.normal(0, 8)
        actual_latency = max(270, min(370, actual_latency))
        ...
        processed_eeg = preprocess_eeg(raw_eeg, fs) # 步骤2：预处理
        avg_erp, epochs = extract_erp(processed_eeg, stim_times, fs) # 步骤3：提取ERP
        p300_amp, p300_lat, erp_waveform = detect_p300(avg_erp, fs) # 步骤4：检测P300

        if p300_amp is not None:  # 步骤5：计算风险评分
    ...
    with col4:
        st.metric("不匹配指数", f"{mismatch:.0f}/100")
    ...
    with col_a:
        ...
        score_color = "score-red" if fraud_score > 850 else "score-orange" if fraud_score > 600 else "score-green"
        st.markdown(f'<p class="score-large {score_color}">{fraud_score:.0f} / 1000</p>', unsafe_allow_html=True)
        # st.markdown(f"<h1 style='text-align: center; color: {'red' if fraud_score > 850  else
        #      'orange' if fraud_score > 600 else 'green'};'>{fraud_score:.0f} / 1000</h1>", unsafe_allow_html=True)

        st.progress(fraud_score / 1000) # 进度条
    ...
    axes[0].plot(time_axis, raw_eeg, 'b-', alpha=0.6, label='原始EEG', linewidth=0.8)
    axes[0].plot(time_axis[:len(processed_eeg)], processed_eeg, 'r-', alpha=0.8, label='预处理后', linewidth=1.2)
    for stim_time in stim_times:    # 标记刺激事件
    ...
    axes[0].set_title('EEG信号预处理 (绿色虚线=刺激时刻)')
    axes[0].legend()
    ...
    if erp_waveform is not None:
        ...
        axes[1].plot(erp_time, erp_waveform, 'b-', linewidth=2.5, label='ERP平均波形')
        if p300_amp is not None: # 标记P300峰值
            p300_time = (p300_lat / 1000) - 0.2
            axes[1].plot(p300_time, p300_amp, 'ro', markersize=12, label=f'P300: {p300_amp:.2f}μV @ {p300_lat:.0f}ms')
            axes[1].axvline(x=p300_time, color='red', linestyle='--', alpha=0.5)
        axes[1].axhline(y=0, color='black', linestyle='-', linewidth=0.5)
        axes[1].set_xlabel('时间 (秒)')
        axes[1].set_ylabel('振幅 (μV)')
        axes[1].set_title('ERP波形 — P300检测结果')
        axes[1].legend()
    ...
    if epochs is not None and len(epochs) > 0:
        ...
        for i in range(min(20, epochs.shape[0])):
            axes[2].plot(epoch_time, epochs[i], 'gray', alpha=0.25, linewidth=0.5)
        if erp_waveform is not None:
            axes[2].plot(epoch_time, erp_waveform, 'r-', linewidth=2.5, label='平均ERP')
    ...
    st.markdown("---")
    st.caption(
        "💡 **分析说明**：绿色虚线标记了每个听觉刺激的呈现时刻。P300是刺激后约300ms出现的正向波峰，"
        "其振幅和潜伏期是评估脑功能状态的核心指标。")
    ...
    axes[0].plot(t_ex, erp_ex, 'b-', linewidth=2.5)
    ...
    # 三种场景对比
    scenarios = ['健康人', '脑震荡', '装病者']
    amps = [20, 13, 18]
    lats = [300, 335, 308]
    colors = ['green', 'red', 'orange']

    for i, (s, a, l, c) in enumerate(zip(scenarios, amps, lats, colors)):
        erp_s = a * np.exp(-((t_ex - l / 1000) ** 2) / (2 * 0.05 ** 2))
    axes[1].plot(t_ex, erp_s, color=c, linewidth=2, label=f'{s}: {a}μV @ {l}ms')

    axes[1].axhline(y=0, color='black', linewidth=0.5)
    axes[1].axvline(x=0, color='gray', linestyle='--', alpha=0.3)
    axes[1].set_xlabel('时间 (秒)')
    axes[1].set_ylabel('振幅 (μV)')
    axes[1].set_title('三种场景P300波形对比')
    axes[1].legend()
    axes[1].grid(True, alpha=0.3)
